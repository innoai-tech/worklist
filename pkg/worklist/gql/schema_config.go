...
func (c *schemaConfigCollector) Register(v any, optionFns ...OptionFunc) {
	...
	case Directive:
		...
		for sf := range collection.IterStructField(context.Background(), rv) {
			...
			if doc, ok := runtimeDoc(v, sf.FieldName); ok {
				arg.Description = strings.Join(doc, "\n")
			}
		...
		if nameConfig != nil {
			if doc, ok := runtimeDoc(x); ok {
				nameConfig.Description = strings.Join(doc, "\n")
			}
...
func (c *schemaConfigCollector) TypeOf(rv reflect.Value, optionFns ...OptionFunc) graphql.Type {
	...
	switch x := v.(type) {
	case enumeration.CanEnumValues:
		enumValues := map[string]*graphql.EnumValueConfig{}

		for _, value := range x.EnumValues() {
			ev := &graphql.EnumValueConfig{}
			ev.Value = value
			if canLabel, ok := value.(interface{ Label() string }); ok {
				ev.Description = canLabel.Label()
			}
			enumValues[fmt.Sprint(value)] = ev
		}
		return c.Enum(rv.Type().Name(), graphql.EnumConfig{
			Values: enumValues,
		})
	case *time.Time:
		return c.Scalar("Timestamp", graphql.ScalarConfig{
			Description: "时间字符串 ISO8601",
		...
		if opt.AsInput {
			t := c.InputObject(tpeName, func() graphql.InputObjectConfig {
				...
				for sf := range collection.IterStructField(context.Background(), rv) {
					...
					if doc, ok := runtimeDoc(v, sf.FieldName); ok {
						field.Description = strings.Join(doc, "\n")
					}
...
func (c *schemaConfigCollector) Enum(name string, config graphql.EnumConfig) graphql.Type {
	if t, ok := c.types[name]; ok {
		return t.Type()
	}
